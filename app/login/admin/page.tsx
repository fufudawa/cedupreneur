...
import { loginWithEmail, requireRole } from "@/lib/auth";
      ...
      <div className="flex flex-1 items-center justify-center px-8 py-16 sm:px-14 lg:px-20">
        <div className="w-full max-w-md rounded-3xl border border-soft-gray-dark bg-white/95 p-6 shadow-[0px_12px_30px_0px_rgba(23,18,46,0.1)] sm:p-8">
          ...
          <div className="mt-6 flex items-center gap-3">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-purple/20 bg-purple/10 text-purple">
              <AdminIcon className="h-6 w-6" />
            </span>
            <h1 className="text-2xl font-extrabold text-navy sm:text-3xl">Login sebagai admin</h1>
          </div>
          <p className="mt-2 text-sm text-muted sm:text-base">
            Admin mengelola pengguna, data master, relasi kelas, dan monitoring sistem.
          </p>

          <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label htmlFor="email" className="text-xs font-bold text-navy sm:text-sm">
                Email
              </label>
              <input
                id="email"
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border-[1.6px] border-purple bg-white px-4 py-3 pr-11 text-sm text-navy outline-none placeholder:text-muted sm:text-base"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword((v) => !v)}
                  aria-label={showPassword ? "Sembunyikan password" : "Tampilkan password"}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-muted transition-colors hover:text-purple"
                >
