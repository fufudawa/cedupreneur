...
import { loginWithNip, requireRole } from "@/lib/auth";
...
export default function DosenLoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await loginWithNip(normalizedNip, password);
      ...
      <div className="flex flex-1 items-center justify-center px-8 py-16 sm:px-14 lg:px-20">
        <div className="w-full max-w-md rounded-3xl border border-soft-gray-dark bg-white/95 p-6 shadow-[0px_12px_30px_0px_rgba(23,18,46,0.1)] sm:p-8">
          ...
          <p className="mt-2 text-sm text-muted sm:text-base">
            Dosen bertugas merancang project
            <br />
            dan mentoring mahasiswa
          </p>

          <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label htmlFor="nip" className="text-xs font-bold text-navy sm:text-sm">
                NIP
              </label>
              <input
                id="nip"
